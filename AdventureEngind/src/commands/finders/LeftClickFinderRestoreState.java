...
public class LeftClickFinderRestoreState extends LeftClickFinder {
	private int xClick;
	private int yClick;
	public IGameObject findObject(IGameModel model, IInputEvent event) {
		...
		for(int i = 0, n = RestoreGameDialog.getInstance().getContainedObjectsCount(); i < n; i++){
			IGameObject current = RestoreGameDialog.getInstance().getContainedObject(i);
			int startX = (int) (current.getX() + RestoreGameDialog.getInstance().getX());
			int endX = (int) (startX + current.getWidth());
			int startY = (int) ( current.getY() + RestoreGameDialog.getInstance().getY());
