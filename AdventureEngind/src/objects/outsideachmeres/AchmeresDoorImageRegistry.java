...
public class AchmeresDoorImageRegistry {
	...
	private AchmeresDoorImageRegistry(){
		try {
			imageMap.put(IGameObject.InternalObjectStates.CLOSED, ImageIO.read(new File(globalvariables.constants.IMAGE_LOCATION+"nineteenth2"+File.separatorChar+"achmeresdoorClosed.bmp")));
			imageMap.put(IGameObject.InternalObjectStates.OPEN, ImageIO.read(new File(globalvariables.constants.IMAGE_LOCATION+"nineteenth2"+File.separatorChar+"achmeresdoorOpen.bmp")));
		} catch (IOException e) {
